                ...
                <div class="card">
                    <form action="{{ route('bills.index') }}" method="GET" class="md-3 d-flex">
                        ...
                        <select name="filter_status">
                            ...
                            <option name="bill_status" value="0" {{ request('filter_status') == '0' ? 'selected' : '' }}>
                                Unconfirmed</option>
                            <option name="bill_status" value="1" {{ request('filter_status') == '1' ? 'selected' : '' }}>
                                Confirmed</option>
                            <option name="bill_status" value="2" {{ request('filter_status') == '2' ? 'selected' : '' }}>
                                Paid</option>
                            <option name="bill_status" value="3" {{ request('filter_status') == '3' ? 'selected' : '' }}>
                                Cancelled</option>
                        </select>

                        <input type="text" class="form-control" placeholder="Enter phonenumber..." name="key"
                            value="{{ request('key') }}">
                        <input type="date" class="form-control" placeholder="Enter date start..." name="filter_date_start"
                            value="{{ request('date_start') }}">
                        <input type="date" class="form-control" placeholder="Enter date finish..."
                            name="filter_date_finish" value="{{ request('date_end') }}">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @foreach ($roomCategories as $category)
                                                @if ($bill->category_id == $category->id)
                                        ...
                                        <td>
                                            ...
                                            <a href="{{ route('bills.edit', $bill->id) }}"><button type="button"
